...
@click.command(help="Run one of the Green Labs.")
...
def launch_lab1(lab_number, labs_folder, SCI, run_in_cpp):
    ...
    if not os.path.exists(lab_path):
        lab_path = f'{labs_folder}/Lab1/Lab1_{decimal}/LAB1_{decimal}.pyc'
    command = f"python {lab_path}"
    if lab_number == "1.2" or (run_in_cpp and lab_number == "1.3"):
        compiled = f"{labs_folder}/Lab1/Lab1_{decimal}/LAB1_{decimal}.bin"
        if not os.path.exists(compiled):
            sys.exit("fatal: file %s does not exist.  Have you compiled it?" % compiled)
        command = compiled
